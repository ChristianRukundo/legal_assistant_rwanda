...
from pydantic_settings import (
    BaseSettings,
    SettingsConfigDict,
)  # Correct import for BaseSettings
...
cache_misses = Counter(
    "cache_misses_total", "Total cache misses", ["cache_type", "key_type"]
)
...
cache_memory_usage = Gauge(
    "cache_memory_usage_bytes", "Cache memory usage in bytes", ["cache_type"]
)
...
class CompressionManager:
    ...
    def compress(data: bytes, compression_type: CompressionType) -> bytes:
        ...
        if compression_type == CompressionType.NONE:
            ...
            logger.warning(
                f"Unknown compression type: {compression_type.value}. Returning uncompressed data."
            )
            return data
    ...
    def decompress(data: bytes, compression_type: CompressionType) -> bytes:
        ...
        if compression_type == CompressionType.NONE:
            ...
            logger.warning(
                f"Unknown decompression type: {compression_type.value}. Returning raw data."
            )
            return data
    ...
    def serialize(obj: Any, serialization_type: SerializationType) -> bytes:
        ...
        try:
            if serialization_type == SerializationType.JSON:
                ...
                logger.warning(
                    f"Unknown serialization type: {serialization_type.value}. Falling back to pickle."
                )
                return pickle.dumps(obj)
        except Exception as e:
            logger.error(
                f"Serialization failed for type {serialization_type.value}: {e}",
                exc_info=True,
            )
            raise
    ...
    def deserialize(data: bytes, serialization_type: SerializationType) -> Any:
        ...
        try:
            if serialization_type == SerializationType.JSON:
                ...
                logger.warning(
                    f"Unknown deserialization type: {serialization_type.value}. Falling back to pickle."
                )
                return pickle.loads(data)
        except Exception as e:
            logger.error(
                f"Deserialization failed for type {serialization_type.value}: {e}",
                exc_info=True,
            )
            raise
...
class MemoryCache:
    ...
    def set(
        ...
        with self._lock:
            ...
            expires_at = (
                datetime.now() + timedelta(seconds=actual_ttl)
                if actual_ttl > 0
                else None
            )
            ...
            logger.debug(
                f"MemoryCache: Set key '{key}'. Current size: {len(self._cache)}"
            )
    ...
    def delete(self, key: str) -> bool:
        ...
        with self._lock:
            ...
            if key in self._cache:
                ...
                cache_operation_duration.labels(
                    operation="delete", cache_type="memory"
                ).observe(time.perf_counter() - start_time)
    ...
    def _estimate_size(self, obj: Any) -> int:
        ...
        try:
            ...
            logger.warning(
                f"MemoryCache: Could not accurately estimate size of object: {e}. Returning default size 100."
            )
            return 100
    ...
    def get_stats(self) -> CacheStats:
        ...
        with self._lock:
            ...
            hit_rate = (
                float(self._stats["hits"]) / total_requests
                if total_requests > 0
                else 0.0
            )
            ...
            oldest_entry: Optional[datetime] = (
                min((e.created_at for e in entries), default=None) if entries else None
            )
            newest_entry: Optional[datetime] = (
                max((e.created_at for e in entries), default=None) if entries else None
            )
...
class RedisCache:
    ...
    def __init__(
        self, config: CacheConfig, redis_client: Optional[aioredis.Redis] = None
    ):
        self.config = config
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._is_initialized = True
            logger.info(
                f"Redis cache initialized successfully. Connected to {self.config.redis_host}:{self.config.redis_port}/{self.config.redis_db}."
            )
            ...
            self.async_redis_client = None
            logger.warning(
                f"Could not connect to Redis at {self.config.redis_host}:{self.config.redis_port}. "
            ...
            self.async_redis_client = None
            logger.error(
                f"An unexpected error occurred while initializing Redis cache: {e}",
                exc_info=True,
            )
    ...
    def _prepare_value(
        self, value: Any, compression: CompressionType, serialization: SerializationType
    ) -> bytes:
        ...
        if (
            ...
            and compression != CompressionType.NONE
            and len(serialized_data) >= self.config.compression_threshold
        ):
    ...
    def _extract_value(self, data: bytes) -> Optional[Any]:
        ...
        try:
            ...
            value = self.serialization_manager.deserialize(
                decompressed_data, serialization
            )
            ...
            logger.error(
                f"Failed to extract value from Redis cache data: {e}", exc_info=True
            )
            return None
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        if not self._is_initialized or self.async_redis_client is None:
            logger.debug(
                "RedisCache not initialized or client not available. Skipping get operation."
            )
            return None
        ...
        try:
            ...
            if value is not None:
                ...
                cache_misses.labels(cache_type="redis", key_type="unknown").inc()
                logger.warning(
                    f"RedisCache: Failed to extract value for key '{key}'. Treating as miss."
                )
            ...
            logger.warning(
                f"Redis connection error during get for key '{key}': {e}. Treating as miss."
            )
    ...
    async def set(
        ...
        if not self._is_initialized or self.async_redis_client is None:
            logger.debug(
                "RedisCache not initialized or client not available. Skipping set operation."
            )
            return False
        ...
        actual_compression = (
            compression if compression is not None else self.config.default_compression
        )
        actual_serialization = (
            serialization
            if serialization is not None
            else self.config.default_serialization
        )

        try:
            start_time = time.perf_counter()
            prepared_value = self._prepare_value(
                value, actual_compression, actual_serialization
            )

            result: Any
            if actual_ttl > 0:
                result = await self.async_redis_client.setex(
                    cache_key, actual_ttl, prepared_value
                )
            else:
    ...
    async def delete(self, key: str) -> bool:
        ...
        if not self._is_initialized or self.async_redis_client is None:
            logger.debug(
                "RedisCache not initialized or client not available. Skipping delete operation."
            )
            return False
    ...
    async def clear_pattern(self, pattern: str) -> int:
        ...
        if not self._is_initialized or self.async_redis_client is None:
            logger.debug(
                "RedisCache not initialized or client not available. Skipping clear_pattern operation."
            )
            return 0
        ...
        try:
            ...
            keys_to_delete = [
                k async for k in self.async_redis_client.scan_iter(match=full_pattern)
            ]

            if keys_to_delete:
                deleted = await self.async_redis_client.delete(*keys_to_delete)
                cache_operations.labels(
                    operation="clear_pattern", cache_type="redis"
                ).inc()
                cache_operation_duration.labels(
                    operation="clear_pattern", cache_type="redis"
                ).observe(time.perf_counter() - start_time)
    ...
    async def clear(self) -> None:
        ...
        if not self._is_initialized or self.async_redis_client is None:
            logger.debug(
                "RedisCache not initialized or client not available. Skipping clear operation."
            )
            return
        ...
        try:
            ...
            logger.info(
                f"RedisCache: Cleared all entries in DB {self.config.redis_db}."
            )
    ...
    async def get_stats(self) -> CacheStats:
        ...
        try:
            ...
            total_items = (
                int(db_info.get("keys", 0)) if isinstance(db_info, dict) else 0
            )
...
class DiskCache:
    ...
    def __init__(
        self, config: CacheConfig, cache_client: Optional[diskcache.Cache] = None
    ):
        self.config = config
    ...
    async def initialize(self) -> None:
        ...
        if self.cache is None:
            try:
                ...
                logger.info(
                    f"Disk cache initialized successfully at '{self.cache_dir}'."
                )
            except Exception as e:
                self.cache = None
                logger.error(
                    f"Failed to initialize disk cache at '{self.cache_dir}': {e}",
                    exc_info=True,
                )
        else:
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            if data is None:
                ...
                cache_operation_duration.labels(
                    operation="get", cache_type="disk"
                ).observe(time.perf_counter() - start_time)
                return None

            cache_hits.labels(cache_type="disk", key_type="unknown").inc()
            cache_operation_duration.labels(operation="get", cache_type="disk").observe(
    ...
    async def set(self, key: str, value: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            cache_operation_duration.labels(operation="set", cache_type="disk").observe(
                time.perf_counter() - start_time
            )
            return result
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            ...
            cache_operation_duration.labels(
                operation="delete", cache_type="disk"
            ).observe(time.perf_counter() - start_time)
            return result
        except Exception as e:
    ...
    async def get_stats(self) -> CacheStats:
        ...
        try:
            ...
            current_hits = cache_hits.labels(
                cache_type="disk", key_type="unknown"
            )._value.get()
            current_misses = cache_misses.labels(
                cache_type="disk", key_type="unknown"
            )._value.get()
    ...
    async def close(self) -> None:
        ...
        if self.cache:
            try:
                self.cache.close()
                logger.info(
                    f"DiskCache: Closed diskcache for directory '{self.cache_dir}'."
                )
            except Exception as e:
...
class MultiLayerCache:
    ...
    def __init__(
        ...
        self.config = config
        self.memory_cache = memory_cache_client or MemoryCache(
            config.memory_cache_size, config.memory_cache_ttl
        )
        ...
        self.cache_layers: List[
            Tuple[Union[MemoryCache, RedisCache, DiskCache], CacheType]
        ] = [
        ...
        for cache_layer, layer_type in self.cache_layers:
            try:
                ...
                if value is not None:
                    ...
                    logger.debug(
                        f"MultiLayerCache: Cache hit for key '{key}' in {layer_type.value} layer."
                    )
                    break

            except Exception as e:
                logger.warning(
                    f"Error accessing {layer_type.value} cache for key '{key}': {e}"
                )
                continue
    ...
    async def _populate_upper_layers(
        self, key: str, value: Any, found_layer: CacheType
    ) -> None:
        ...
        for cache_layer, layer_type in reversed(self.cache_layers):
            ...
            try:
                ...
                logger.debug(
                    f"MultiLayerCache: Populated key '{key}' to {layer_type.value} layer."
                )
            except Exception as e:
                logger.warning(
                    f"Failed to populate upper cache layer {layer_type.value} for key '{key}': {e}"
                )
    ...
    async def set(
        ...
        value: Any,
        ttl: Optional[int] = None,
        layers: Optional[List[CacheType]] = None,
    ) -> bool:
        ...
        target_layers = (
            ...
            else [CacheType.MEMORY, CacheType.REDIS, CacheType.DISK]
        )
        ...
        for cache_layer, layer_type in self.cache_layers:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to set in {layer_type.value} cache for key '{key}': {e}"
                )
    ...
    async def delete(self, key: str, layers: Optional[List[CacheType]] = None) -> bool:
        ...
        for cache_layer, layer_type in self.cache_layers:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to delete from {layer_type.value} cache for key '{key}': {e}"
                )
                results.append(False)
    ...
    async def get_combined_stats(self) -> Dict[str, CacheStats]:
        ...
        for cache_layer, layer_type in self.cache_layers:
            try:
                ...
                logger.warning(
                    f"Failed to get stats from {layer_type.value} cache: {e}"
                )
...
class CacheManager:
    ...
    def __init__(
        ...
        self.cache = MultiLayerCache(
            self.config, memory_cache_client, redis_cache_client, disk_cache_client
        )
    ...
    async def initialize(self) -> None:
        ...
        if not self._background_tasks_started:
            ...
            cleanup_task = asyncio.create_task(
                self._cleanup_expired_entries(), name="CacheCleanupTask"
            )
            metrics_task = asyncio.create_task(
                self._update_metrics(), name="CacheMetricsTask"
            )
            ...
            cleanup_task.add_done_callback(
                lambda t: self.background_tasks.discard(t)
                or self._check_background_tasks_completion()
            )
            metrics_task.add_done_callback(
                lambda t: self.background_tasks.discard(t)
                or self._check_background_tasks_completion()
            )
        ...
        try:
            ...
            if not force_refresh:
                ...
                if fallback_value is not None:
                    logger.warning(
                        f"Returning stale value for key '{key}' due to computation failure."
                    )
    ...
    async def warm_cache(self, warming_config: Dict[str, Dict[str, Any]]) -> None:
        ...
        for key, config in warming_config.items():
            ...
            if compute_func:
                ...
                logger.warning(
                    f"Cache warming config for key '{key}' missing 'compute_func'. Skipping."
                )

        if warming_tasks:
            logger.info(
                f"CacheManager: Waiting for {len(warming_tasks)} cache warming tasks to complete..."
            )
    ...
    async def _cleanup_expired_entries(self) -> None:
        ...
        while self._running.is_set():
            try:
                logger.debug(
                    "CacheManager: Running periodic expired entry cleanup check (no explicit action in this layer)."
                )
                ...
                logger.error(
                    f"Error in cache cleanup background task: {e}", exc_info=True
                )
        ...
        while self._running.is_set():
            try:
                ...
                for layer_name, layer_stats in stats.items():
                    cache_size_gauge.labels(cache_type=layer_name).set(
                        layer_stats.total_items
                    )
                    cache_memory_usage.labels(cache_type=layer_name).set(
                ...
                logger.error(
                    f"Error updating cache metrics background task: {e}", exc_info=True
                )
                await asyncio.sleep(60)
    ...
    async def invalidate_by_tags(self, tags: Set[str]) -> None:
        ...
        logger.info(
            f"CacheManager: Invalidating cache entries with tags: {tags} (Not fully implemented in current layers)."
        )

    async def get_health_status(self) -> Dict[str, Any]:
        """Get cache health status"""
        health = {"status": "healthy", "layers": {}, "errors": []}

        health["layers"]["memory"] = "healthy"

        if self.cache.redis_cache._is_initialized and (
            redis_client := self.cache.redis_cache.async_redis_client
        ):
            try:
    ...
    async def close(self) -> None:
        ...
        if self._background_tasks_started:
            ...
            try:
                ...
                logger.warning(
                    "CacheManager: Timeout waiting for background tasks to complete."
                )
            logger.info("CacheManager: All background tasks stopped.")

        if self.cache_warming_tasks:
            logger.warning(
                f"CacheManager: Cancelling {len(self.cache_warming_tasks)} outstanding warming tasks."
            )
    ...
    async def get_transcription_cache(
        ...
        return cast(
            Optional[Dict[str, Any]],
            await self.cache.get(f"transcription:{audio_hash}"),
        )
...
async def cache_get_or_compute(
    ...
    manager = await get_cache_manager()
    return await manager.get_or_compute(
        key, compute_func, ttl=ttl, force_refresh=force_refresh
    )

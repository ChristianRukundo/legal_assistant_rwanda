...
class DocumentIngestionService:
    ...
    async def process_and_index_document(self, file_path: Path):
        ...
        if not file_path.exists():
            logger.warning(f"File not found: {file_path}, attempting to delete from knowledge base.")
        ...
        try:
            ...
            async with await get_db_connection() as db:
                ...
                existing_doc = await db.execute(
                    "SELECT id, file_hash FROM documents WHERE file_path = ?", (str(file_path),)
                )
                doc_row = await existing_doc.fetchone()

                if doc_row and doc_row["file_hash"] == file_hash:
                    logger.info(f"Document {file_path} is already up to date. Skipping.")
                    return

                
                if doc_row:
                    logger.info(f"Document {file_path} has been modified. Re-indexing...")
                    await self._delete_document_data(db, doc_row["id"])

                
                processed_doc = await self.doc_processor.process_document(str(file_path))
                ...
                async with db.executescript("BEGIN TRANSACTION;"):
                    if processed_doc.metadata.status == ProcessingStatus.COMPLETED:
                        ...
                        chunk_data = [
                            (doc_id, chunk.content, chunk.page_number, chunk.chunk_index)
                        ...
                        if chunk_data:
                            ...
                            chunk_rows = await db.execute("SELECT id FROM chunks WHERE doc_id = ?", (doc_id,))
                            chunk_ids = [row['id'] for row in await chunk_rows.fetchall()]

                            
                            chunk_contents = [chunk.content for chunk in processed_doc.chunks]
                            embeddings = self.embedding_manager.get_embeddings(chunk_contents)
                            await self.vector_store_manager.add(embeddings, chunk_ids)
                    else:
                        logger.warning(f"Processing failed for {file_path}. Status: {processed_doc.metadata.status.value}")
                        await db.execute(
                            "INSERT INTO documents (file_path, file_hash, status) VALUES (?, ?, ?)",
                            (str(file_path), file_hash, processed_doc.metadata.status.value)
                        )
    ...
    async def delete_document(self, file_path: str):
        ...
        async with await get_db_connection() as db:
            async with db.executescript("BEGIN TRANSACTION;"):
                cursor = await db.execute("SELECT id FROM documents WHERE file_path = ?", (file_path,))
                doc_row = await cursor.fetchone()
                if doc_row:
                    await self._delete_document_data(db, doc_row["id"])
                    logger.info(f"Successfully deleted document: {file_path}")
                else:
                    logger.warning(f"Document not found in knowledge base for deletion: {file_path}")

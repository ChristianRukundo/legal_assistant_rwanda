"""
Advanced Document Processing System for Inyandiko Legal AI Assistant
...
try:
    ...
    logging.warning("spaCy 'en_core_web_sm' model not found. NER and some NLP features will be disabled.")
    nlp = None
...
doc_processing_counter = Counter('docproc_documents_processed_total', 'Total documents processed', ['status', 'type'])
doc_processing_duration = Histogram('docproc_processing_duration_seconds', 'Document processing duration')
...
memory_usage_gauge = Gauge('docproc_memory_mb', 'Memory usage in MB during document processing')
...
class DocumentType(Enum):
    ...
    MARKDOWN = "md"; XLSX = "xlsx"; CSV = "csv"; JSON = "json"
    XML = "xml"; EPUB = "epub"; ZIP = "zip"; RAR = "rar"
    SEVEN_Z = "7z"; IMAGE = "image"; UNKNOWN = "unknown"

class ProcessingStatus(Enum):
    PENDING = "pending"; PROCESSING = "processing"; COMPLETED = "completed"
    FAILED = "failed"; CORRUPTED = "corrupted"; UNSUPPORTED = "unsupported"
    SECURITY_RISK = "security_risk"

class ChunkingStrategy(Enum):
    FIXED_SIZE = "fixed_size"; SENTENCE_BASED = "sentence_based"
    PARAGRAPH_BASED = "paragraph_based"; SEMANTIC_BASED = "semantic_based"
    HIERARCHICAL = "hierarchical"; TOPIC_BASED = "topic_based"

@dataclass
class DocumentMetadata:
    """Comprehensive, structured metadata for a processed document."""
    file_path: str; file_name: str; file_size: int; file_hash: str
    mime_type: str; document_type: DocumentType; created_at: datetime
    modified_at: datetime; processed_at: Optional[datetime] = None
    language: Optional[str] = None; page_count: Optional[int] = None
    word_count: Optional[int] = None; character_count: Optional[int] = None
    reading_level: Optional[float] = None; sentiment: Optional[Dict[str, float]] = None
    summary: Optional[str] = None; keywords: List[str] = field(default_factory=list)
    topics: List[str] = field(default_factory=list); entities: List[Dict] = field(default_factory=list)
    pii_detected: List[Dict] = field(default_factory=list)
    security_scan_result: Optional[Dict] = None; processing_time: Optional[float] = None
    resource_usage: Optional[Dict[str, float]] = None; error_message: Optional[str] = None
    status: ProcessingStatus = ProcessingStatus.PENDING

@dataclass
class DocumentChunk:
    """A structured representation of a single chunk of document content."""
    chunk_id: str; content: str; chunk_index: int; start_position: int; end_position: int
    page_number: Optional[int] = None; section_title: Optional[str] = None
    chunk_type: str = "text"; confidence_score: float = 1.0
...
class SecurityScanner:
    ...
    def __init__(self, config: DocumentProcessorConfig):
        ...
        self.blocked_extensions = {'.exe', '.bat', '.cmd', '.scr', '.pif', '.com', '.vbs'}
        self.suspicious_patterns = [re.compile(p, re.IGNORECASE) for p in [
            r'javascript:', r'vbscript:', r'<script', r'eval\(', r'document\.write', r'window\.location']]
    ...
    async def scan_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_size > self.max_file_size:
                scan_result.update({'safe': False, 'threats': [f"File exceeds size limit of {self.config.max_file_size_mb}MB"]})
                return scan_result

            if Path(file_path).suffix.lower() in self.blocked_extensions:
                scan_result.update({'safe': False, 'threats': ["File has a blocked extension."]})
                return scan_result
                ...
                async with aiofiles.open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    for pattern in self.suspicious_patterns:
                        if pattern.search(content_sample):
                            scan_result['warnings'].append(f"Suspicious pattern found: {pattern.pattern}")
...
class OCRProcessor:
    ...
    async def extract_text_from_image_async(self, image_bytes: bytes, lang: str = 'eng') -> Dict[str, Any]:
...
class TextAnalyzer:
    ...
    def __init__(self, config: DocumentProcessorConfig):
        ...
        try:
            ...
            logging.warning(f"Could not load SentenceTransformer model: {e}. Semantic features will be disabled.")
            self.sentence_model = None
        self.kw_extractor = yake.KeywordExtractor(lan="en", n=3, dedupLim=0.9, top=self.config.max_keywords, features=None)
    ...
    def detect_language(self, text: str) -> str:
        try: return detect(text) if len(text.strip()) > 20 else 'unknown'
    ...
    def analyze_sentiment(self, text: str) -> Dict[str, float]:
        ...
        except Exception: return {'polarity': 0.0, 'subjectivity': 0.0}

    def summarize_text(self, text: str) -> str:
        try:
            sentences = sent_tokenize(text)
            if len(sentences) <= self.config.max_summary_sentences: return " ".join(sentences)
            vectorizer = TfidfVectorizer(stop_words='english')
            tfidf_matrix = vectorizer.fit_transform(sentences)
            sentence_scores = tfidf_matrix.toarray().sum(axis=1).flatten()  # type: ignore
            top_indices = sentence_scores.argsort()[-self.config.max_summary_sentences:][::-1]
            summary_sentences = [sentences[i] for i in sorted(top_indices)]
            return " ".join(summary_sentences)
        except Exception: return text[:500] 

    def extract_keywords(self, text: str) -> List[str]:
        try: return [kw for kw, score in self.kw_extractor.extract_keywords(text)]
    ...
    def calculate_similarity(self, text1: str, text2: str) -> float:
        if not self.sentence_model:
            return float(textdistance.jaccard.similarity(word_tokenize(text1), word_tokenize(text2)))
        try:
            embeddings = self.sentence_model.encode([text1, text2])
            return float(cosine_similarity(np.array(embeddings[0]).reshape(1, -1), np.array(embeddings[1]).reshape(1, -1))[0][0])
...
class ChunkingEngine:
    ...
    def chunk(self, strategy: ChunkingStrategy, text: str, structure: Optional[Dict] = None) -> List[DocumentChunk]:
        """Selects and applies a chunking strategy."""
        if strategy == ChunkingStrategy.TOPIC_BASED:
            return self._chunk_by_topic(text)
        if strategy == ChunkingStrategy.HIERARCHICAL and structure and structure.get("toc"):
        ...
        for i, p in enumerate(paragraphs):
            try:
                ...
                chunks.append(DocumentChunk(
                    chunk_id=f"para_{i}", content=p, chunk_index=i,
                    start_position=start_pos, end_position=start_pos + len(p)
                ))
            except ValueError:
                continue
        return chunks
    ...
    def _chunk_by_fixed_size(self, text: str) -> List[DocumentChunk]:
        ...
        while start < len(text):
            ...
            if content:
                chunks.append(DocumentChunk(
                    chunk_id=f"fixed_{index}", content=content, chunk_index=index,
    ...
    def _chunk_hierarchically(self, text: str, structure: Dict) -> List[DocumentChunk]:
        ...
        if not toc: return self._chunk_by_paragraphs(text)
        
        content_map = []
        for i, (level, title, page_num) in enumerate(toc):
            start_pos = text.find(title)
            if start_pos != -1:
                content_map.append({'title': title, 'start': start_pos, 'page': page_num})
        
        content_map.sort(key=lambda x: x['start'])

        for i, section in enumerate(content_map):
            start_pos = section['start']
            end_pos = content_map[i+1]['start'] if i + 1 < len(content_map) else len(text)
            content = text[start_pos:end_pos].strip()
            
            if len(content) > 50:
                chunks.append(DocumentChunk(
                    chunk_id=f"hier_{i}", content=content, chunk_index=len(chunks),
                    start_position=start_pos, end_position=end_pos,
                    section_title=section['title'], page_number=section['page']
                ))
        return chunks
    ...
    def _chunk_by_topic(self, text: str) -> List[DocumentChunk]:
        ...
        try:
            ...
            lda = LatentDirichletAllocation(n_components=self.config.num_topics_for_chunking, random_state=42)
            topic_assignments = lda.fit_transform(X).argmax(axis=1)
            
            chunks, current_chunk_paras, current_topic = [], [], topic_assignments[0]
            for i, para in enumerate(paragraphs):
                if topic_assignments[i] == current_topic and len("\n\n".join(current_chunk_paras)) < 4000:
                    current_chunk_paras.append(para)
                else:
                    content = "\n\n".join(current_chunk_paras)
                    chunks.append(DocumentChunk(
                        chunk_id=f"topic_{len(chunks)}", content=content, chunk_index=len(chunks),
                        start_position=text.find(content), end_position=text.find(content) + len(content),
            ...
            logging.warning(f"Topic-based chunking failed: {e}. Falling back to paragraphs.")
            return self._chunk_by_paragraphs(text)
...
class BaseExtractor:
    ...
    def __init__(self, config: DocumentProcessorConfig, ocr_processor: Optional[OCRProcessor] = None):
        self.config = config
    ...
    async def extract(self, file_path: str) -> Tuple[str, Dict, List, List, List, List]:
        ...
        with fitz.Document(file_path) as doc:
            ...
            for i in range(len(doc)):
                if i >= self.config.max_pages_per_doc: break
                page = doc.load_page(i)
                text_builder.append(page.get_text())
                links.extend([link['uri'] for link in page.get_links() if 'uri' in link])
                annotations.extend([annot.info['content'] for annot in page.annots() if annot.info.get('content')])
            ...
            for i, page in enumerate(pdf.pages):
                ...
                if page_tables:
                    for tbl in page_tables:
                        tables.append({'page': i + 1, 'data_html': pd.DataFrame(tbl[1:], columns=tbl[0]).to_html()})
...
class DocxExtractor(BaseExtractor):
    async def extract(self, file_path: str) -> Tuple[str, Dict, List, List, List, List]:
        try:
            ...
            text_builder, tables = [], []
            for para in doc.paragraphs: text_builder.append(para.text)
            for table in doc.tables:
                table_data = [[cell.text for cell in row.cells] for row in table.rows]
                tables.append({'page': None, 'data_html': pd.DataFrame(table_data).to_html()})
                text_builder.append(pd.DataFrame(table_data).to_string())
            
            props = doc.core_properties
            structure = {'author': props.author, 'title': props.title, 'created': props.created}
...
class SpreadsheetExtractor(BaseExtractor):
    async def extract(self, file_path: str) -> Tuple[str, Dict, List, List, List, List]:
        ...
        for sheet_name in xls.sheet_names:
            ...
            if not df.empty:
                text_builder.append(f"--- SHEET: {sheet_name} ---\n{df.to_string()}\n\n")
                tables.append({'page': sheet_name, 'data_html': df.to_html()})
        return "\n".join(text_builder).strip(), {"sheets": xls.sheet_names}, [], tables, [], []
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                if file_path.endswith('.zip'):
                    with zipfile.ZipFile(file_path, 'r') as zf: zf.extractall(temp_dir)
                elif file_path.endswith('.rar'):
                    with rarfile.RarFile(file_path, 'r') as rf: rf.extractall(temp_dir)
                elif file_path.endswith('.7z'):
                    with py7zr.SevenZipFile(file_path, mode='r') as z: z.extractall(temp_dir)
            except Exception as e:
            ...
            text_builder.append(f"Archive contains files: {', '.join(structure['files'])}")
...
class AdvancedDocumentProcessor:
    ...
    def _detect_doc_type(self, file_path: str) -> Tuple[DocumentType, str]:
        mime_type_map = {
            'application/pdf': DocumentType.PDF, 'text/plain': DocumentType.TXT,
            'application/vnd.openxmlformats-officedocument.wordprocessingml.document': DocumentType.DOCX,
            'text/html': DocumentType.HTML, 'text/markdown': DocumentType.MARKDOWN,
            'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': DocumentType.XLSX,
            'application/zip': DocumentType.ZIP, 'application/x-rar-compressed': DocumentType.RAR,
        ...
        except Exception: return DocumentType.UNKNOWN, "application/octet-stream"
    ...
    async def _create_metadata(self, file_path: str) -> DocumentMetadata:
        ...
        metadata = DocumentMetadata(
            ...
            file_size=stat.st_size, file_hash=self._calculate_file_hash(file_path),
            mime_type=mime, document_type=doc_type,
    ...
    async def process_document(
        self, file_path: str, chunking_strategy: ChunkingStrategy = ChunkingStrategy.HIERARCHICAL
    ) -> ProcessedDocument:
        ...
        metadata = DocumentMetadata(file_path, Path(file_path).name, 0, "", "application/octet-stream", DocumentType.UNKNOWN, datetime.now(), datetime.now())
        text = ""
        structure: Optional[Dict] = None
        images: List[Dict] = []
        tables: List[Dict] = []
        ...
        try:
            ...
            if not extractor:
                raise NotImplementedError(f"No extractor for type {metadata.document_type.value}")

            metadata.status = ProcessingStatus.PROCESSING
            text, structure, images, tables, links, annotations = await extractor.extract(file_path)

            if not text.strip():
                metadata.status = ProcessingStatus.FAILED
                metadata.error_message = "No text content extracted"
                return ProcessedDocument(metadata, [], text, structure=structure, images=images, tables=tables, links=links, annotations=annotations)

            loop = asyncio.get_running_loop()
            text_analysis = await loop.run_in_executor(self.executor, self.text_analyzer.analyze, text)
            ...
            doc_processing_counter.labels(status=metadata.status.value, type=metadata.document_type.value).inc()

        processed_doc = ProcessedDocument(
            metadata=metadata, chunks=chunks, full_text=text, redacted_text=redacted_text,
            structure=structure, images=images, tables=tables, links=links, annotations=annotations
        )
    ...
    async def to_json(doc: ProcessedDocument, output_path: str):
        class CustomEncoder(json.JSONEncoder):
            def default(self, o):
                ...
                if isinstance(o, bytes): return "<binary_data>"
...
def print_processing_report(doc: ProcessedDocument):
    ...
    try:
        ...
        console = Console()
        status_color = {"COMPLETED": "green", "FAILED": "red", "SECURITY_RISK": "yellow"}.get(doc.metadata.status.value, "white")
        ...
        meta_table.add_row("Status:", f"[{status_color}]{doc.metadata.status.value}[/{status_color}]")
        if doc.metadata.error_message: meta_table.add_row("Error:", f"[red]{doc.metadata.error_message}")
        meta_table.add_row("Type:", f"{doc.metadata.document_type.value} ({doc.metadata.mime_type})")
        meta_table.add_row("Language:", str(doc.metadata.language))
        meta_table.add_row("Counts:", f"{doc.metadata.word_count} words, {doc.metadata.character_count} chars")
        if doc.metadata.processing_time is not None:
             meta_table.add_row("Processing Time:", f"{doc.metadata.processing_time:.3f}s")
        if doc.metadata.resource_usage:
             meta_table.add_row("Memory Usage:", f"{doc.metadata.resource_usage['mem_usage_mb']:.2f} MB")
        ...
        if doc.metadata.status == ProcessingStatus.COMPLETED:
            console.print(Panel(analysis_text, title="[green]Text Analysis[/green]", border_style="green", expand=False))
            console.print(Panel(chunks_text, title="[yellow]Chunking[/yellow]", border_style="yellow", expand=False))
        ...
        if doc.metadata.error_message: print(f"Error: {doc.metadata.error_message}")


# ==============================================================================
# 8. SELF-CONTAINED TEST HARNESS
# ==============================================================================

def create_sample_files(temp_dir: str) -> List[str]:
    """Creates a variety of sample files for comprehensive testing."""
    # PDF
    pdf_path = os.path.join(temp_dir, "sample_report.pdf")
    doc = fitz.Document()  # type: ignore
    page = doc.new_page()  # type: ignore
    page.insert_text((50, 72), "Section 1: The Constitution", fontsize=16)
    page.insert_text((50, 100), "The constitution provides the supreme law of the land. It defines the framework of the government. My email is test@example.com.")
    doc.set_toc([ [1, "Section 1: The Constitution", 1] ])  # type: ignore
    doc.save(pdf_path)
    
    # DOCX
    docx_path = os.path.join(temp_dir, "corporate_memo.docx")
    doc = DocxDocument(); doc.add_heading('Corporate Law Overview', 0)
    doc.add_paragraph('This document covers key aspects of corporate governance. Call us at (555) 123-4567 for more info.')
    doc.save(docx_path)
    
    # XLSX
    xlsx_path = os.path.join(temp_dir, "case_data.xlsx")
    pd.DataFrame({'Case ID': [101, 102], 'Verdict': ['Guilty', 'Not Guilty']}).to_excel(xlsx_path, index=False)
    
    # TXT
    txt_path = os.path.join(temp_dir, "witness_testimony.txt")
    with open(txt_path, "w") as f: f.write("Plain text file containing witness testimony.")

    # ZIP
    zip_path = os.path.join(temp_dir, "legal_archive.zip")
    with zipfile.ZipFile(zip_path, 'w') as zf:
        for file in [pdf_path, docx_path, xlsx_path, txt_path]:
            zf.write(file, os.path.basename(file))
        
    logging.info(f"Created sample files in {temp_dir}")
    return [pdf_path, docx_path, xlsx_path, txt_path, zip_path]

async def main():
    """Main function to demonstrate and test the full capabilities of the processor."""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        logging.info("\n>>> Starting batch processing of all sample files...")
        results = await processor.batch_process(sample_files)
        logging.info(">>> Batch processing complete. Generating reports and JSON output...\n")
        
        for result in results:
            print_processing_report(result)
            json_output_path = os.path.join(output_dir, f"{result.metadata.file_name}.json")
            await ResultExporter.to_json(result, json_output_path)
        
        logging.info(f"Structured JSON outputs saved to: {output_dir}")

...
from intelligent_query_processor import IntelligentQueryProcessor, ProcessedQuery, QueryComplexity, EmotionalTone
...
class DocumentGraphBuilder:
    ...
    def build_document_graph(self, documents: List[Document]) -> nx.DiGraph:
        ...
        doc_nodes = [n for n, data in self.graph.nodes(data=True) if data.get('type') == 'document']
    ...
    async def retrieve(self, query_context: ProcessedQuery) -> RetrievalResult:
        ...
        if self.config.enable_document_graph_analysis and self.embedding_manager._parent_rag_pipeline is not None \
           ...
           and query_context.entities:
            ...
            query_entity_texts = [ent['text'] for ent in query_context.entities if isinstance(ent, dict) and 'text' in ent]
...
class AdvancedRAGPipeline:
    ...
    async def enhanced_query(self, query: str, language: str, query_analysis: ProcessedQuery, query_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.enable_document_graph_analysis and self.document_graph_builder.is_initialized and query_analysis.entities:
                
                query_entity_texts = [ent['text'] for ent in query_analysis.entities if isinstance(ent, dict) and 'text' in ent]
                if query_entity_texts:
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            health_status = False  # Placeholder, implement real check if available
        except Exception as e:
